...
# optimizer
optimizer = dict(type='SGD', lr=0.00354, momentum=0.9, weight_decay=0.0001) # 使用 随机梯度下降（SGD）作为优化器.学习率（learning rate）设置为 0.0025。动量（momentum）设置为 0.9，用于加速 SGD 的收敛。权重衰减（weight decay）设置为 0.0001，用于正则化，防止过拟合。
...
lr_config = dict(
    ...
    warmup_iters=1500, # 预热阶段的迭代次数为 1500。
